...
class EventCalendarTests(TestCase):
    ...
    def test_group_by_day(self):
        ...
        cal = EventCalendar(2025, 11, events)
        self.assertIn(1, cal.events_by_day)
    ...
    def test_formatday(self):
        ...
        now = datetime(2025, 11, 28)
        events = [self.DummyEvent(now.day, "Test Event")]
        cal = EventCalendar(now.year, now.month, events)

        # День 0 (пустой)
        empty_html = cal.formatday(0, 0)
        self.assertIn("noday", empty_html)

        # День с событием
        html = cal.formatday(now.day, 4)  # 4 = пятница (по cssclasses)
